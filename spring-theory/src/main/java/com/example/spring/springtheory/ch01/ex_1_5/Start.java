...
import com.example.spring.springtheory.ch01.ex_1_5.domain.User;
...
// 스프링에서는 빈의 생성과 관계설정 같은 제어를 담당하는 IoC 오브젝트(컴포넌트,컨테이너도 괜찮)를 빈 팩토리라고한다.
...
// - 클라이언트는 구체적인 팩토리 클래스를 알 필요가 없다 <- 99% 메인에서 한줄로 첫 설정을 하는 것은 불가피하지만(scan어노테이션을 쓰면 해결) 의존성을 매우 줄였기 때문에 이렇게 표현한다
