...
public class BoardRepositoryImpl implements BoardRepositoryCustom {
    ...
    public Page<BoardListItemResponseDto> searchBoards(BoardSearchRequestDto condition, Pageable pageable) {
        ...
        // 순서일치를 안해주면 이상한 값이 들어갈 수 있기 때문에 조심
...
/*        // 2페이지를 요청하고(0부터 시작하므로 1), 한 페이지에 10개씩 가져옴
        Pageable pageable = PageRequest.of(1, 10);

// 이 순간 내부적으로 offset은 (1 * 10) = 10으로 설정됨
        long myOffset = pageable.getOffset(); // 결과: 10*/
        ...
        // 3) 메서드 참조
        return PageableExecutionUtils.getPage(content, pageable, countQuery::fetchOne); //totalCount같은 것을 알려면 카운트 쿼리가 필요하다. << 이것이 디폴트라서 필수
    }
    ...
    private Expression<Long> commentCountOf(QBoard board) {
        return JPAExpressions
                ...
                .from(comment)
                .where(comment.board.id.eq(board.id)); //실제 필드인 id를 찾고 board객체를 갖고 옴(여기선 해당안함 id로 바로 비교하기 때문 title같이 한번 바텀업이 수행되어야 하는 것 만 기능 수행)
    }
