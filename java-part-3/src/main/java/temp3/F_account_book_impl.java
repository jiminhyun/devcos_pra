...
public class F_account_book_impl implements F_account_book{
    ...
    public void showAccount() {
        ...
        Collections.sort(dates, Collections.reverseOrder());  //디폴트는 오름차순, 내림차순
    ...
    public void deleteItem() {
        ...
        if (no < 1 || no > items.size()) {
            System.out.println("번호 제대로 입력하세요");
            return;
        }

        F_item removed = items.remove(no - 1);
        System.out.println(removed.getName() + "삭제 되었습니다.");

        if(items.isEmpty()) {
            data.remove(date);
            System.out.println("(" + date + ") 의 항목 모두 사라졌기 때문에 날짜도 삭제됩니다.");
        }
    }
